...
# --- 快取函數 (效能優化) ---
@st.cache_data(ttl=300) # 分鐘級數據建議縮短快取時間 (例如 5 分鐘)
def load_stock_data(ticker, start_date, interval):
    try:
        # yfinance 分鐘級數據限制: 1m (7天), 2m-30m (60天), 60m-90m (730天)
        # 這裡我們做個防呆，如果是分鐘級，start_date 不能太早
        
        if interval.endswith('m'): # 如果是分鐘級別
            # 確保不會超過 API 限制 (這裡保守抓 59 天)
            limit_date = datetime.now() - timedelta(days=59)
            # 如果使用者選的日期早於限制，強制設為限制日期
            # 注意: start_date 從 date_input 進來可能是 date 物件，需轉為 datetime 比較
...
interval_option = st.sidebar.selectbox(
    "選擇 K 線週期",
    options=["日線 (1 Day)", "5 分鐘 (5 Minutes)"],
    index=0
)
...
# 修改 2: 根據週期動態調整預設日期與最小日期
if interval == "5m":
    # 5分鐘數據限制為最近 59 天 (保留一點緩衝)
    min_start_date = datetime.now() - timedelta(days=59)
    default_start = datetime.now() - timedelta(days=5) # 預設看最近5天
    st.sidebar.info("⚠️ 注意：5分鐘數據最多只能回測最近 59 天。")
else:
    # 日線數據可以從很早開始
...
def calculate_strategy(df, sell_threshold_val, use_sl, trailing_val, commission): # 移除了 sl_val (硬性停損參數)
    ...
    # 優化：只在開頭或跨年時顯示年份，其餘隱藏年份以節省空間
    new_index = []
    for i in range(len(df)):
        curr_dt = df.index[i]
        show_year = (i == 0) or (curr_dt.year != df.index[i-1].year)
        ...
        new_index.append(curr_dt.strftime(fmt))
    ...
    with st.spinner(f'正在運算 {ticker} 的交易策略...'):
        try:
            ...
            if df.empty:
                ...
                # 5分鐘線顯示最近 300 根 (約3-4天)
                # 日線顯示 250 根 (約1年)
                show_bars = 300 if interval == "5m" else 250
